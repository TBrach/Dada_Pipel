...
# TbTresult_list_show <- lapply(TbTresult_list, function(df) {
#         select(df, Family, Genus, Species, 2:19, 22)
# })

# df <- head(TbTresult_list_show[["Young_vs_Old"]], 25)
# df <- select(df, c(1:2, 4, 8, 12:13))
# colnames(df)[4:6] <- c("fisher", "mean_young", "mean_old")
# print(xtable(df, align = "|c|c|c|c|c|c|c|", digits = 4), include.rownames = FALSE)
# row_names_heat <- paste(df$Family, df$Genus)
# 
# df <- TbTresult_list[["Young_vs_Old"]]
# top_tbt <- as.character(df$Taxon[1:25])
# ps_tca_filt_tbt <- prune_taxa(top_tbt, ps_tca_filt)
# ps_filt_ra_tbt <- prune_taxa(top_tbt, ps_filt_ra)
# 
# overview_list_tbt <- overviewPhyseq2(ps_filt_ra_tbt, group_var = group_var, max_rel_ab_for_color = .05, tax_order = top_tbt, tax_names = row_names_heat)
# pdf(file = "tbt_heat.pdf", width = 10, height = 8)
# overview_list_tbt[[2]]
# dev.off()
# 
# tbt_violins <- plot_top_abundances_boxAndviolin(ps_tca_filt_tbt, group_var = group_var, tax_order = top_tbt, tax_names = row_names_heat)
# 
# pdf(file = "tbt_violin.pdf", width = 10, height = 9)
# tbt_violins[[2]][[4]]
# dev.off()
# 
# pdf(file = "tbt_violin_log.pdf", width = 10, height = 9)
# tbt_violins[[2]][[8]]
# dev.off()
```
...
# for (i in 1:length(TbTresult_list_show)){
#         print(knitr::kable(head(TbTresult_list_show[[i]], 20), caption = names(TbTresult_list_show)[i]))
# }
```
...
# 
# table(tax_table(ps_filt_ra_extra)[, "Phylum"])
```
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
# overview_list <- overviewPhyseq(ps_tca_filt_extra, group_var = group_var, max_rel_ab_for_color = NULL)
# overview_list1 <- overviewPhyseq(ps_tca_filt_extra, group_var = group_var, max_rel_ab_for_color = .1)
```
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
# diff_prevalence <- test_differential_prevalence(physeq = ps_tca_filt_extra, group_var = group_var, p.adj.method = "fdr", minCount = 0L)
# level_position <- which(colnames(diff_prevalence[[1]]) == extra_level)
# diff_prevalence_show <- lapply(diff_prevalence, function(df) {
#         select(df, c(level_position -1 , level_position), 2:7)
# })
# ```
# 
# ```{r, warning = FALSE, message = FALSE, echo = FALSE, results = "asis"}
...
# head_values[head_values == -Inf] <- 10
# head_values[head_values > 35] <- 35
# for (i in 1:length(diff_prevalence_show)){
#         print(knitr::kable(head(diff_prevalence_show[[i]], head_values[i]), caption = names(diff_prevalence_show)[i]))
# }
# 
# 
# df <- head(diff_prevalence_show[["Young_vs_Old"]], 17)
# df <- select(df, -direction, -p_val_Young_vs_Old)
# colnames(df)[3:4] <- c("adj_p_val", "signi.")
# print(xtable(df, align = "|c|c|c|c|c|c|c|", digits = 4), include.rownames = FALSE)
# row_names_heat <- paste(df$Family, df$Genus)
# 
# df <- diff_prevalence[["Young_vs_Old"]]
# top_fisher <- df$Taxon[1:17]
# ps_filt_ra_fisher <- prune_taxa(top_fisher, ps_filt_ra_extra)
# 
# overview_list_fisher <- overviewPhyseq2(ps_filt_ra_fisher, group_var = group_var, max_rel_ab_for_color = .1, tax_order = top_fisher, tax_names = row_names_heat)
# pdf(file = "fisher_heat_genus.pdf", width = 10, height = 8)
# overview_list_fisher[[2]]
# dev.off()
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
# DESeq2_result_list <- DESeq2Apply_physeq(physeq = ps_filt_extra, group_var = group_var, SFs = SFs, p.adjust.method = "fdr")
# level_position <- which(colnames(DESeq2_result_list[[1]]) == extra_level)
# DESeq2_result_list_show <- lapply(DESeq2_result_list, function(df) {
#         select(df, c(level_position - 1, level_position), 2:24)
# })
```
...
# suppressWarnings(head_values <- sapply(DESeq2_result_list, function(df){
#         max(which(df$p_val_adj < 0.05))
# })) # in case of no significant you get a warning from max resulting in -Inf
# head_values <- head_values + 5
# head_values[head_values == -Inf] <- 10
# head_values[head_values > 35] <- 35
# for (i in 1:length(DESeq2_result_list_show)){
#         print(knitr::kable(head(DESeq2_result_list_show[[i]], head_values[i]), caption = names(DESeq2_result_list_show)[i]))
# }
# 
# df <- head(DESeq2_result_list_show[["Young_vs_Old"]], 25)
# df <- select(df, c(1:2, 5:6, 10, 14:15))
# colnames(df)[3:7] <- c("adj_p_val", "signi.", "fisher", "mean_young", "mean_old")
# print(xtable(df, align = "|c|c|c|c|c|c|c|c|", digits = 4), include.rownames = FALSE)
# row_names_heat <- paste(df$Family, df$Genus)
# 
# df <- DESeq2_result_list[["Young_vs_Old"]]
# top_DES <- as.character(df$Taxon[1:25])
# ps_tca_filt_DES <- prune_taxa(top_DES, ps_tca_filt_extra)
# ps_filt_ra_DES <- prune_taxa(top_DES, ps_filt_ra_extra)
# 
# overview_list_DES <- overviewPhyseq2(ps_filt_ra_DES, group_var = group_var, max_rel_ab_for_color = .1, tax_order = top_DES, tax_names = row_names_heat)
# pdf(file = "DES_heat_genus.pdf", width = 10, height = 8)
# overview_list_DES[[2]]
# dev.off()
# DES_violins <- plot_top_abundances_boxAndviolin(ps_tca_filt_DES, group_var = group_var, tax_order = top_DES, tax_names = row_names_heat)
# 
# pdf(file = "DES_violin_genus.pdf", width = 10, height = 9)
# DES_violins[[2]][[4]]
# dev.off()
# 
# pdf(file = "DES_violin_log_genus.pdf", width = 10, height = 9)
# DES_violins[[2]][[8]]
# dev.off()
```
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
# TbT_tiles <- create_TbT_TilePlots(TbTmatrixes_list, physeq = ps_filt_phylum, group_var = group_var)
# TbT_tiles_show <- lapply(TbT_tiles, `[[`, 2)
```
