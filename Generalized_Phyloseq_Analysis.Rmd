...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
diff_prevalence <- test_differential_prevalence(physeq = ps_taa_filt, group_var = group_var, p.adj.method = "fdr", minCount = 0L)
...
for (i in 1:length(diff_prevalence_show)){
        print(knitr::kable(head(diff_prevalence_show[[i]], 10), caption = names(diff_prevalence_show)[i]))
}
```