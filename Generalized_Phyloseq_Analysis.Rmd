...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
overview_list <- overviewPhyseq(ps_tca_filt, group_var = group_var, max_rel_ab_for_color = NULL)
overview_list1 <- overviewPhyseq(ps_tca_filt, group_var = group_var, max_rel_ab_for_color = .1)
```
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
diff_prevalence <- test_differential_prevalence(physeq = ps_tca_filt, group_var = group_var, p.adj.method = "fdr", minCount = 0L)
diff_prevalence_show <- lapply(diff_prevalence, function(df) {
        select(df, Family, Genus, Species, 2:7)
})
```

```{r, warning = FALSE, message = FALSE, echo = FALSE, results = "asis"}
head_values <- sapply(diff_prevalence, function(df){
        max(which(df[, 3] < 0.05))
})
...
wilcox_ra_result_list_show <- lapply(wilcox_ra_result_list, function(df) {
        select(df, Family, Genus, Species, 2:22)
})
...
head_values[head_values > 35] <- 35
for (i in 1:length(wilcox_ra_result_list_show)){
        print(knitr::kable(head(wilcox_ra_result_list_show[[i]], head_values[i]), caption = names(wilcox_ra_result_list_show)[i]))
}
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
DESeq2_result_list <- DESeq2Apply_physeq(physeq = ps_filt, group_var = group_var, SFs = SFs, p.adjust.method = "fdr")

DESeq2_result_list_show <- lapply(DESeq2_result_list, function(df) {
        select(df, Family, Genus, Species, 2:24)
})
```
