...
# - beta diversity -
dist_methods <- c("jsd") # otions: see unlist(distanceMethodList)
# --
...
# order the concentration/shape column
# sample_data(ps)$Conc <- factor(sample_data(ps)$Conc, levels = c("1", "2", "5", "15"), ordered = TRUE)
...
# plot_list = list()
alpha_div_wrap <- alpha_diversity_wrapper(physeq = ps, alpha_div_measures = alpha_div_measures, group_var = group_var, shape = shape)
...
# plot_list[[1]] <- TrListBP[["Richness"]]
# plot_list[[2]] <- TrList_lm[["Richness"]]
```
...
alpha_div_wrap <- alpha_diversity_wrapper(physeq = ps.rare, alpha_div_measures = alpha_div_measures, group_var = group_var, shape = shape)
...
# dev.off()
# plot_list[[3]] <- rarefaction_curves[["Tr_richness_col"]]
```
...
if (alpha_at_genus){
        ...
        alpha_div_wrap <- alpha_diversity_wrapper(physeq = ps_gen, alpha_div_measures = alpha_div_measures, group_var = group_var, shape = shape)
        DF_alpha_no_rare <- alpha_div_wrap[["DF_alpha_list"]]$DF_alpha # used later for comparison plots to rarefied version
        TrListBP <- alpha_div_wrap[["TrListBP"]]
        TrList_lm <- alpha_div_wrap[["TrList_lm"]]
        TrList_lm_filt <- alpha_div_wrap[["TrList_lm_filteredReads"]]
...
pcoas <- calc_ordination_from_distances(ps_filt_ra, dist_list = dist_list, ordination_type = "PCoA", group_var = group_var, shape = shape, coord_cor = TRUE) 
...
- listing the most differently "abundant" taxa for all level combinations
...
# print(xtable(DES_table_list[[2]][, 1:5], align = "|c|c|c|c|c|c|c|c|c|c|", digits = 4), include.rownames = FALSE)
...
# pdf(file = "heat_family_DES.pdf", width = 10, height = 8)
# # kable(dist_analysis_list[["DistancePValues"]][[1]])
# heat_maps_DES[[2]]
# dev.off()
```
...
# plot_list[[5]] <- Tr_bar
# saveRDS(object = plot_list, file = "plot_list.rds")
```
...
```{r, fig.width = 14, fig.height = 7, warning = FALSE, message = FALSE, echo = FALSE}
# pdf(file = "bar_phylum.pdf", width = 7, height = 6)
Tr_bar
# dev.off()
```

```{r, fig.width = 14, fig.height = 7, warning = FALSE, message = FALSE, echo = FALSE}
# pdf(file = "bar_phylum2.pdf", width = 7, height = 6)
Tr_bar + coord_cartesian(ylim = c(0.9, 1))
# dev.off()
```
...
# pdf(file = "heat_phylum.pdf", width = 8, height = 6)
# Tr
# dev.off()
```
...
original_head_values <- data.frame(Comparison = c(names(head_values), "Total"), NoSignificant = c(head_values, ntaxa(ps_filt_extra)), PC_Significant = 100*c(head_values, ntaxa(ps_filt_extra))/ntaxa(ps_filt_extra))
head_values[head_values < 10] <- min(10, ntaxa(ps_filt_extra))
head_values[head_values > 25] <- 25
...
heat_maps_DES <- lapply(1:length(pruned_physeqs), function(i){
        maps <- make_heat_map_physeq_levels(pruned_physeqs[[i]], group_var = group_var, max_abundance_for_color = 2000, tax_order = tax_orders[[i]], tax_names = row_names_for_heat_maps[[i]], color_sample_names = TRUE, gradient_steps = c(0.15, 0.3, 0.45, 1))
        maps[[i]]
})
...
# pdf(file = "violin_phylum_DES.pdf", width = 10, height = 8)
# # kable(dist_analysis_list[["DistancePValues"]][[1]])
# violin_plots_DES[[2]]
# dev.off()
```
...
# pdf(file = "Firmicutes_Ratios.pdf", width = 10, height = 8)
# # kable(dist_analysis_list[["DistancePValues"]][[1]])
...
TbT_tiles <- create_raw_TbT_TilePlots(raw_TbTmatrixes_list, physeq = ps_filt_extra, group_var = group_var, tax_names = taxNames, test = "wilcoxon", p_adjust = "none")
