...
# beta diversity (non-rarefied data!)

## Preprocessing

- filtering on prevalence (count > min_obs)
- go to relative abundance


```{r, warning = FALSE, message = FALSE, echo = FALSE}
# # filter taxa
# ps.filt <- filter_taxa(ps, function(x){(sum(x > min_obs) > (prevalence/100)*length(x))}, prune = TRUE)
# 
# # adjust library size (total reads per sample) a la DESEQ
# standf = function(x, t=median(sample_sums(ps.filt))) round(t * (x / sum(x)))
# ps.filt.norm = transform_sample_counts(ps.filt, standf)
# 
# # DO alpha diversity again here!
# 
# # change to relative abundance
# ps.filt.norm.RA = transform_sample_counts(ps.filt.norm, function(x){x/sum(x)})
# ps.filt.RA <- transform_sample_counts(ps.filt, function(x){x/sum(x)})
# 
