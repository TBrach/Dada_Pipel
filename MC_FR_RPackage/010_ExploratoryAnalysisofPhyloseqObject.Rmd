...
- there are `r ntaxa(ps)` taxa and `r nsamples(ps)` samples in the phyloseq object
...
- Table \@ref(tab:phyla_before_adjust) shows an overview of the phyla representation in the data set.
...
```{r phyla_before_adjust, echo = FALSE, message=FALSE}
knitr::kable(Phyla)
```
...
- **NB: only if you chose "yes" for "phylum_NA_removal"**
