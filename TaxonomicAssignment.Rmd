...
AmpliconDistribution <- as.data.frame(table(colSums(seqtab != 0)))
colnames(AmpliconDistribution) <- c("InNoSamples", "Amplicons")
# add the precentage reached when leaving sequences in that are in at least n samples
AmpliconDistribution$CumSum <- rev(cumsum(rev(AmpliconDistribution$Amplicons)))
AmpliconDistribution$CumPerC <- rev(cumsum(rev(AmpliconDistribution$Amplicons/ncol(seqtab))))

# Distribution plot
Tr <- ggplot(AmpliconDistribution, aes(x = InNoSamples, y = Amplicons))
Tr <- Tr + geom_point(col = "#E69F00", size = 3) +
        xlab("In Number of Samples") +
        theme_bw() +
        theme(panel.grid.minor = element_blank(),
              panel.grid.major.y = element_blank(),
              panel.grid.major.x = element_line(color = "#999999", size = .15))

# CumpPerC plot: CAN be used to decide on how many samples an Amplicon needs to present to stay (assuming you want to keep a certain % of amplicons)
Tr1 <- ggplot(AmpliconDistribution, aes(x = InNoSamples, y = CumPerC))
